...
static void WriteCommand(uint8_t command)
{
	...
	HAL_SPI_Transmit(&hspi3, &command, 1U, 100U);
}

static void WriteData(uint8_t data)
{
	HAL_GPIO_WritePin(ILI9341_DC_PORT, ILI9341_DC_PIN, GPIO_PIN_SET);
	HAL_SPI_Transmit(&hspi3, &data, 1U, 100U);
}
...
void ILI9341Pixel(uint16_t x, uint16_t y, colour_t colour)
{
	...
	HAL_SPI_Transmit(&hspi3, (uint8_t *)&beColour, 2U, 100UL);
}
...
{
	txComplete = false;
	HAL_SPI_Transmit_DMA(&hspi3, (uint8_t *)data, length);
}
